...
trainer_slide("YOUR TRAINER",C.TRAINER,"Principal Trainer\nTertiary Infotech Academy Pte. Ltd.",
 [("Role","Principal Trainer, Tertiary Infotech Academy Pte. Ltd."),
  ...
  ("Organisation",C.ORG)],
 initials=_trainer_initials,accent=BLUE)
...
_right=[(f"Day {_d2} — {C.DAY_THEMES[_d2]}",0)]+[_topic_line(_TB[n]) for n in _BY_DAY.get(_d2,[]) if n in _TB]+[
 ("Daily timing",0),(getattr(C,"DAILY_TIMING","9:30 am–6:30 pm · 1-hour lunch · tea breaks within training time"),1)]
_daily_hours=getattr(C,"DAY_MINUTES",480)/60
two_col(f"Lesson Plan — {C.DAYS} Day{'s' if C.DAYS>1 else ''}, {_daily_hours:g} hours/day",_left,_right,
...
if _OVERVIEW:
    ...
    if _OVERVIEW.get("pillars"):
        tile_grid(_OVERVIEW.get("pillars_title","What You'll Build"),
                  [(t," · ".join(b)) for t,b in _OVERVIEW["pillars"]],
                  kicker="COURSE ROADMAP",cols=2,size=14)
...
for t in C.TOPICS:
    ...
    # Rich concept teaching comes from the same knowledge sections used in the
    # Learner Guide. Each section gets an explanation slide and a visual model
    # slide before learners see the related labs.
    for ks in t.get("knowledge_sections",[]):
        content(ks["title"],ks.get("body",[]),kicker=ks.get("kicker","CONCEPT"),size=16)
        if ks.get("visual"):
            tile_grid(f"Visual Model — {ks['title']}",ks["visual"],
                      kicker=ks.get("kicker","CONCEPT MODEL"),cols=2,size=13,accent=accent)
    ...
    for a in acts:
        ...
        _lab_extras(a["num"])
        # The deck teaches the objective, context, build path, and verification.
        # Click-by-click detail and prompts remain in the LG and lab Markdown.
        # This keeps concept/knowledge teaching above 60% of the deck.
        key_steps=a.get("deck_steps",[si for si,_sc in a["steps"][:3]])
        content(f"Build Path & Verification — Lab {a['num']}",
                [f"{i}. {step}" for i,step in enumerate(key_steps,1)]
                + ["Verify: "+a.get("deck_test",a["test"])],
                kicker=f"LAB {a['num']} · KEY STEPS",size=15)
    # topic recap
