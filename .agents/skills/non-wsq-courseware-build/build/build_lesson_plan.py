...
def keep_row_together(row):
    """Prevent LibreOffice/Word from splitting a schedule row across pages."""
    trPr=row._tr.get_or_add_trPr()
    if trPr.find(qn("w:cantSplit")) is None:
        trPr.append(OxmlElement("w:cantSplit"))

def repeat_header(row):
    """Repeat a table header if an unforeseen pagination change spans pages."""
    trPr=row._tr.get_or_add_trPr()
...
schedule_heading=H("Course Schedule",1)
schedule_heading.paragraph_format.page_break_before=True
for day_index,(day,(theme,rows)) in enumerate(SCHEDULE.items()):
    H(f"Day {day} — {theme}",2)
    day_heading=doc.paragraphs[-1]
    if day_index:
        day_heading.paragraph_format.page_break_before=True
    tbl=doc.add_table(rows=0,cols=3); tbl.style="Table Grid"; tbl.alignment=WD_TABLE_ALIGNMENT.CENTER
    header_row=tbl.add_row(); keep_row_together(header_row); repeat_header(header_row)
    ...
    for start,end,mins,kind,text in rows:
        ...
        cells=body_row.cells; fill=KIND_FILL.get(kind)
...
lab_reference_heading=H("Lab Reference (aligned to course topics)",1)
lab_reference_heading.paragraph_format.page_break_before=True
tt=doc.add_table(rows=0,cols=3); tt.style="Table Grid"
header_row=tt.add_row(); keep_row_together(header_row); repeat_header(header_row)
hdr=header_row.cells
...
for tp in C.TOPICS:
    ...
    body_row=tt.add_row(); keep_row_together(body_row)
