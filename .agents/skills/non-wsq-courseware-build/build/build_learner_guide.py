...
def visual(title,items): B.append(("visual",title,items))
...
for t in C.TOPICS:
    ...
    p(t["subtitle"])
    for paragraph in t.get("overview",[]):
        p(paragraph)
    h3("Key concepts")
    bullets(t["concepts"])
    for section in t.get("knowledge_sections",[]):
        h2(section["title"])
        for paragraph in section.get("body",[]):
            p(paragraph)
        if section.get("visual"):
            visual("Concept visual",section["visual"])
    for a in [x for x in ACT if x["topic"]==t["num"]]:
        h2(f"Lab {a['num']} — {a['title']}")
        p(f"Learning outcome: {a['objective']}.")
        p(f"Goal: {a['desc']}")
        h3("What you'll build")
        p(a["build"]+f"   (Tools: {a['services']}; duration: {a.get('duration','trainer-guided')}.)")
        h3("Prerequisites")
        bullets(a.get("prerequisites",["Complete the earlier labs in sequence."]))
        h3("Step-by-step")
        ...
        p(a.get("checkpoint",a["build"]))
        if a.get("troubleshooting"):
            h3("Troubleshooting")
            bullets([f"{symptom}: {fix}" for symptom,fix in a["troubleshooting"]])
        h3("Challenge")
        p(a.get("challenge","Extend the artifact with one additional evidence-backed scenario."))
        h3("Reflection")
        p(a.get("reflection","What evidence shows that this artifact is ready for the next step?"))
        note(f"Full step-by-step instructions, prompts, and verification checks are in labs/lab-{a['num']:02d}-*.md. "
...
def render_md():
    ...
    for kind,*rest in B:
        if kind=="h1": out+=["",f"## {rest[0]}",""]
            ...
            out += [f"**{rest[0]}**","","| Element | Meaning |","|---|---|"]
            out += [f"| {label} | {body} |" for label,body in rest[1]] + [""]
        elif kind=="dl":
...
for kind,*rest in B:
    if kind=="h1":
        heading=doc.add_heading(rest[0],level=1)
        heading.paragraph_format.keep_with_next=True
    elif kind=="h2":
        heading=doc.add_heading(rest[0],level=2)
        heading.paragraph_format.keep_with_next=True
    elif kind=="h3":
        ...
        para=doc.add_paragraph(); r=para.add_run(rest[0]); r.bold=True; r.font.color.rgb=BRAND
        para.paragraph_format.keep_with_next=True
        tbl=doc.add_table(rows=1,cols=2); tbl.style="Table Grid"
        keep_row_together(tbl.rows[0]); repeat_header(tbl.rows[0])
        tbl.rows[0].cells[0].text="Element"; tbl.rows[0].cells[1].text="Meaning"
        for cell in tbl.rows[0].cells:
            prodoc._shade_cell(cell,"E8F0FE")
            for run in cell.paragraphs[0].runs: run.bold=True
        for label,body in rest[1]:
            body_row=tbl.add_row(); keep_row_together(body_row)
            cells=body_row.cells; cells[0].text=label; cells[1].text=body
    elif kind=="dl":
